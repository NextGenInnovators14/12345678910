...
export const CategoryPills: React.FC<CategoryPillsProps> = ({ 
  ...
  return (
    <div className="bg-[#FAF7F2] border-b border-[#E5DEC9] py-1.5 sm:py-2 shadow-2xs w-full max-w-full overflow-hidden transition-colors">
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 w-full max-w-full">
        <div className="flex items-center space-x-1 sm:space-x-2 overflow-x-auto no-scrollbar py-0.5 scroll-smooth -mx-2.5 px-2.5 sm:mx-0 sm:px-0 w-full min-w-0">
          
          {/* 3-Dot All Features Quick Trigger */}
          <button
            onClick={() => onOpenFeaturesMenu?.()}
            className="group flex items-center space-x-1 sm:space-x-1.5 px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-xl sm:rounded-2xl text-[11px] sm:text-xs font-black whitespace-nowrap transition-all duration-200 border shrink-0 bg-amber-100 hover:bg-[#1E4FA8] text-[#1E4FA8] hover:text-white border-amber-300/80 hover:border-[#1E4FA8] shadow-2xs cursor-pointer active:scale-95"
            title="Open 3-Dot Features & Navigation Hub"
          >
            <div className="p-0.5 sm:p-1 rounded-lg bg-white text-[#F2621E] shadow-2xs group-hover:bg-[#F2621E] group-hover:text-white transition-colors duration-200 shrink-0">
              <MoreVertical className="w-3 h-3 sm:w-3.5 sm:h-3.5 rotate-90 sm:rotate-0" />
            </div>
            <span>All Features</span>
            <span className="text-[8px] sm:text-[9px] px-1 py-0.2 sm:px-1.5 sm:py-0.5 rounded-full font-black bg-[#F2621E] text-white">
              ...
            </span>
          </button>
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`group flex items-center space-x-1.5 sm:space-x-2 px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-xl sm:rounded-2xl text-[11px] sm:text-xs font-bold whitespace-nowrap transition-all duration-200 border shrink-0 cursor-pointer ${
                  cat.highlight
              ...
              >
                <div className={`p-0.5 sm:p-1 rounded-lg sm:rounded-xl transition-colors duration-200 shrink-0 ${
                ...
                <span>{cat.label}</span>
                <span className={`text-[9px] sm:text-[10px] px-1 sm:px-1.5 py-0.2 sm:py-0.5 rounded-md font-semibold ${
                  cat.highlight || isSelected
                    ? 'bg-white/20 text-white'
