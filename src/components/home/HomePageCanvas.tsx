...
export const HomePageCanvas: React.FC<Props> = ({ editMode = false, selectedId, onSelect, onOpenEMIModal, onOpenAffiliateModal, onOpenAdvertiseModal }) => {
  ...
  return <>
    {homePageConfig.sections.map(section => {
      ...
      // The approved homepage wireframe keeps the homepage focused on the core
      // conversion journey. These legacy standalone rows are already represented
      // inside the compact Projects/Properties layouts or are outside the PDF scope.
      if (['whyChoose'].includes(section.id)) return null;
