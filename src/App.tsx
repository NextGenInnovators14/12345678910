...
const BrokerLogin = lazy(() => import('./components/broker/BrokerLogin').then(m => ({ default: m.BrokerLogin })));
const AffiliatePortal = lazy(() => import('./components/affiliate/AffiliatePortal').then(m => ({ default: m.AffiliatePortal })));
const AffiliateLogin = lazy(() => import('./components/affiliate/AffiliateLogin').then(m => ({ default: m.AffiliateLogin })));
const AffiliateDashboardView = lazy(() => import('./components/affiliate/AffiliateDashboardView').then(m => ({ default: m.AffiliateDashboardView })));
...
const AppContent: React.FC = () => {
  ...
  // SPA navigation must always start at the top of the destination page.
  // Keep browser scroll restoration under application control; otherwise
  // mobile Chrome may restore the previous route's scroll position after
  // pushState(), which can make a shorter destination appear to open at its
  // footer.
  useLayoutEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--text-primary)] flex flex-col font-sans transition-colors duration-200 pb-16 md:pb-0">
      ...
      <main className="flex-1 w-full max-w-full overflow-x-hidden">
        ...
        {activeView === 'broker-login' && <BrokerLogin />}
        ...
        {activeView === 'affiliate-login' && <AffiliateLogin />}
        {activeView === 'affiliate-dashboard' && <AffiliateDashboardView />}
        {activeView === 'affiliate-status' && <AffiliatePortal mode="status" />}
        {activeView === 'affiliate-admin' && <AffiliatePortal mode="admin" />}

        {cmsPages?.[activeView] && !['home','properties','projects','commercial','plots','rentals','pgs','about','property-detail','project-detail','service-detail','services','offers','offer-detail','blogs','contact','legal','valuator','post-property','realtors','knowledge-hub','broker-knowledge-hub','admin-hub','broker-hub','broker-login','register','signin','affiliate-login','affiliate-dashboard','affiliate-register','affiliate-landing'].includes(activeView) && (
