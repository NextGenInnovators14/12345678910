...
export const normalizeHomePageConfig = (parsed: Partial<HomePageConfig> | null | undefined): HomePageConfig => {
  const incoming = parsed && Array.isArray(parsed.sections) ? parsed.sections.filter(section => section.id !== 'news') : [];
  const byId = new Map(incoming.map(section => [section.id, section]));
  const merged = DEFAULT_HOME_PAGE_CONFIG.sections.map(def => ({ ...def, ...(byId.get(def.id) || {}) }));
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const viewFromPath = (path: string): string => {
    ...
    const known: Record<string, string> = {
      ...
      '/admin': 'admin-hub', '/admin-hub': 'admin-hub', '/broker': 'broker-hub', '/broker-hub': 'broker-hub', '/broker-login': 'broker-login', '/brokers': 'broker-landing', '/broker-register': 'broker-register', '/broker-status': 'broker-status', '/broker-dashboard': 'broker-dashboard', '/admin/brokers': 'broker-admin', '/affiliate': 'affiliate-landing', '/affiliate-register': 'affiliate-register', '/service-providers': 'service-provider', '/service-provider': 'service-provider', '/service-provider-register': 'service-provider-register', '/service-provider-status': 'service-provider-status', '/admin/service-providers': 'service-provider-admin', '/affiliate-status': 'affiliate-status', '/affiliate-dashboard': 'affiliate-dashboard', '/admin/affiliates': 'affiliate-admin', '/register': 'register', '/signin': 'signin'
    };
  ...
  const pathForView = (view: string): string => {
    const known: Record<string, string> = {
      ...
      realtors: '/realtors', 'knowledge-hub': '/knowledge-hub', 'broker-knowledge-hub': '/broker-knowledge-hub',
      'admin-hub': '/admin', 'broker-hub': '/broker', 'broker-login': '/broker-login', 'broker-landing': '/brokers', 'broker-register': '/broker-register', 'broker-status': '/broker-status', 'broker-dashboard': '/broker-dashboard', 'broker-admin': '/admin/brokers', 'affiliate-landing': '/affiliate', 'affiliate-register': '/affiliate-register', 'affiliate-status': '/affiliate-status', 'affiliate-dashboard': '/affiliate-dashboard', 'affiliate-admin': '/admin/affiliates', register: '/register', signin: '/signin', 'service-provider': '/service-providers', 'service-provider-register': '/service-provider-register', 'service-provider-status': '/service-provider-status', 'service-provider-admin': '/admin/service-providers'
    };
  ...
  // Always reset the document scroll before changing SPA views.
  // Mobile Chrome can preserve the previous page's scroll offset when
  // pushState() changes the URL. If the destination is shorter, that offset
  // lands near its footer. Resetting BEFORE the render + using manual browser
  // scroll restoration prevents the "new page opens from the footer" bug.
  const setActiveView = (view: string) => {
    ...
    // A second reset catches browser scroll restoration that happens after
    // the click handler has returned (common on Android/mobile browsers).
